...
- **`/printtimeout`** — set, show, or clear a per-project override of the Antigravity (`agy`) `--print-timeout` from Telegram: `/printtimeout` (show effective value + global default), `/printtimeout 30m` / `/printtimeout 1h30m` (set, Go duration syntax, lenient client-side validation), `/printtimeout clear` (revert to global default). Persists to `[projects.<alias>].print_timeout` using the same read→validate→atomic-write→hot-apply recipe as `/clone`/`/project`; resolved per-run into the `EngineRunOptions` ContextVar (the same channel `model` uses) and consumed by `AntigravityRunner._resolved_print_timeout()`. Always available (no section-enabled gate, modeled on `/model`); degrades to a plain hint when used outside a project topic. Currently affects Antigravity runs only — a project whose engine isn't `antigravity` still accepts the write but gets a note in the confirmation reply
...
| `telegram/print_timeout.py` | `/printtimeout` command — per-project Antigravity print-timeout set/show/clear, out-of-topic degrade |
