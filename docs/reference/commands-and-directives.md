...
| `/clone <repo-url> [--dir <path>] [@<branch>]` | Clone a repo and register it as a project. `<repo-url>` accepts `https://github.com/OWNER/REPO[.git]` or `git@github.com:OWNER/REPO[.git]`; the host must be in `[clone] allowed_hosts` (default `["github.com"]`). `--dir <path>` picks a destination under the configured `[clone] root` instead of the default; `@<branch>` clones that branch. On success, Untether registers `[projects.<alias>]` (alias derived from the repo name) and, in a forum-enabled group in scope, creates a topic bound to the new project — see [Projects: bootstrap a repo with /clone](../how-to/projects.md#bootstrap-a-repo-from-telegram-with-clone). In private chats or non-forum/topics-disabled groups, the command degrades to register-only and suggests the equivalent `/topic` command. Host git credentials (existing SSH keys, credential helpers) are used as-is — no token is injected. See [`[clone]` config](config.md#clone). |
| `/project <name>` | Register a brand-new *empty* local project (no `git clone`). `<name>` is sanitized into a `[a-z0-9_]` alias; unlike `/clone` the alias is **not** deduped — if it already maps to a registered project the command refuses and reports the existing path. On success, Untether creates an empty `<[new_project] root>/<alias>` directory, registers `[projects.<alias>]`, and — in a forum-enabled group in scope — creates a topic bound to the new project — see [Projects: bootstrap a new project with /project](../how-to/projects.md#bootstrap-a-new-project-from-telegram-with-project). In private chats or non-forum/topics-disabled groups, the command degrades to register-only and suggests the equivalent `/topic` command. See [`[new_project]` config](config.md#new_project). |
